...
def set_headers(func):
    ...
    def decorated_function(*args, **kwargs):
        ...
        response.headers['token'] = global_params.get('token', '')
        return response
...
def add_user():
    """新增用户接口"""
    get_token = {'token': global_params.get('token')}
    ...
    if all([username, password, role_id, dates, phone]) and token and token == get_token['token']:
